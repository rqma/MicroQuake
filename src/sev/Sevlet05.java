...
public class Sevlet05 extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
		if(MainThread.aDataRec[4]!=null) {
		aDataRec=MainThread.aDataRec[4];
		...
		for (int i = 0; i < aDataRec.getAfterVector().size(); i++) {
			/*if(aDataRec.getAfterVector()== null||aDataRec.getAfterVector().size()>=1000||i>=aDataRec.getAfterVector().size()){
				System.out.println("array index out of range------555555555555");
			}
			else{*/
				//System.out.println("getAfterVector05:"+aDataRec.getAfterVector().size());
